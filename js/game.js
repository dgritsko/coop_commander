(function () {
  ...
  function beginGame() {
    ...
    gameState.activeRats.forEach(function (r) {
      var ratTypes = _.filter(RatTypes, function (t) {
        return r.class == t.class && r.size == t.size;
      });

      if (ratTypes.length == 1) {
        var spawnTime = r.spawn * 1000;
        game.time.events.add(spawnTime, function () {
          createRat(ratTypes[0], r.x, r.y, r.speed);
        });
      } else {
  ...
  function beginIntro() {
    ...
    var introLabel = game.add.bitmapText(
      game.world.centerX,
      game.world.centerY,
      "blackOpsOne",
      "Ready!",
  ...
  function beginOutro() {
    ...
    game.time.events.add(1500, function () {
      ...
      game.camera.onFadeComplete.add(function () {
        ...
        var itemInfos = _.map(items, function (i) {
          return {
            id: i.info.id,
            x: i.position.x,
            y: i.position.y,
  ...
  function playGame() {
    ...
    var targets = food.children.map(function (item) {
    ...
    game.physics.arcade.collide(
      ...
      function (rodent, foodItem) {
        var rat = _.find(rats, function (r) {
          return r.id == rodent.id;
        });
    ...
    var levelComplete =
      gameState.inactiveRats.length == gameState.activeRats.length;
  ...
  function attackRat(rodent) {
    ...
    if (gameState.blood) {
      var vector = Phaser.Point.subtract(
        rat.sprite.position,
      ...
      var emitter = game.add.emitter(
        rat.sprite.position.x,
        rat.sprite.position.y,
        10
      );
  ...
  function drawMenu() {
    ...
    var menu = GameUtil.drawKey(
      game,
      game.world.centerX - menuWidth / 2,
      game.world.centerY - menuHeight / 2,
      menuWidth,
      menuHeight
    );

    var continueButton = GameUtil.drawTextButton(
      game,
      menuWidth * 0.25 - 30,
      60,
      "Continue",
    ...
    var quitButton = GameUtil.drawTextButton(
      game,
      menuWidth * 0.75 - 44,
      60,
      "Quit",
    ...
    var spaceKey = GameUtil.drawKey(
      game,
      50,
      keyY + keySize + keySeparator,
      keySize * 5,
      keySize
    );
    ...
    var flashlightKey = GameUtil.drawKey(
      game,
      130,
      firstRowY,
      keySize,
    ...
    var upKey = GameUtil.drawKey(
      game,
      keyX + keySize + keySeparator,
      keyY,
      keySize,
      keySize,
      "↑"
    ...
    var downKey = GameUtil.drawKey(
      game,
      keyX + keySize + keySeparator,
      keyY + keySeparator + keySize,
      keySize,
      keySize,
      "↓"
    );
    ...
    var rightKey = GameUtil.drawKey(
      game,
      keyX + keySize * 2 + keySeparator * 2,
      keyY + keySeparator + keySize,
      keySize,
      keySize,
      "→"
  ...
  function createFood() {
    ...
    var foodIndexes = [
      0, 12, 13, 14, 15, 16, 18, 21, 23, 24, 25, 26, 27, 28, 29, 32, 33, 34, 35,
      36, 37, 38, 40, 43, 44, 46, 47, 48, 49, 50, 56, 59, 61, 62, 63,
    ];
    ...
    spawns.forEach(function (spawn) {
      game.time.events.add(1000 * spawn.time, function () {
        ...
        game.time.events.add(1000 * lifetime, function () {
          powerup.kill();
          powerups = _.filter(powerups, function (p) {
            return p !== powerup;
          });
        });
      });
